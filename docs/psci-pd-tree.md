...
`platform_core_pos_by_mpidr(mpidr)` will return the core index for the core
...
Another mandatory API, `platform_my_core_pos()` has been added to return the core
...
for a core domain will be the same as the index returned by
 `platform_core_pos_by_mpidr()` or `platform_my_core_pos()` for that core. This
...
`platform_my_core_pos()` function. When a core is turned on, the normal world
provides an MPIDR. The `platform_core_pos_by_mpidr()` function is used to validate
