...
{
	uint64_t suspend_start_time, resume_time, delta;
	...
	suspend_start_time = (uint64_t)(((uint64_t)gtc.suspend.sec_hi << 32) + gtc.suspend.sec_lo);
	resume_time = (uint64_t)(((uint64_t)gtc.resume.sec_hi << 32) + gtc.resume.sec_lo);
