...
/**
 * \brief Sentinel value for invalid or non-existent device clock ID
 *
 ...
 * dev_clk_idx_t use this value to signal errors or missing clocks.
 */
...
#define DEV_CLK_PARENT(base, id, parent, _div, _idx)[(base) + (id)] =	\
		{ .clk = (parent),					\
		  .type = DEV_CLK_TABLE_TYPE_PARENT,			\
		  .div = (_div) + (int) BUILD_ASSERT_OR_ZERO(_div < 256), \
		  .idx = (_idx),					\
...
#define DEV_CLK_OUTPUT(base, id, child)[(base) + (id)] =		\
		{ .clk = (child),					\
		  .type = DEV_CLK_TABLE_TYPE_OUTPUT,			\
		  .modify_parent_freq = true }
 ...
 * that if there is only one clock attached to the device, a struct ti_device
 * is actually returned. Both struct ti_dev_clk and struct ti_device contain a
 * single uint8_t flags field. The flags are mutually exclusive.
 */
struct ti_dev_clk *get_dev_clk(struct ti_device *dev, dev_clk_idx_t clk_idx);
 ...
 * invalid.
 */
const struct ti_dev_clk_data *get_dev_clk_data(struct ti_device *dev, dev_clk_idx_t clk_idx);
 ...
 * clock exists.
 */
struct ti_clk *dev_get_clk(struct ti_device *dev, dev_clk_idx_t clk_idx);
 ...
 *
 * \return
 * The current state of the gated flag as set by ti_device_clk_set_gated.
 */
...
#else
static inline void ti_device_clk_enable(struct ti_device *dev, dev_clk_idx_t clk_idx)
{
}
static inline void ti_device_clk_disable(struct ti_device *dev, dev_clk_idx_t clk_idx)
{
}
static inline void ti_device_clk_init(struct ti_device *dev, dev_clk_idx_t clk_idx)
{
}
#endif
