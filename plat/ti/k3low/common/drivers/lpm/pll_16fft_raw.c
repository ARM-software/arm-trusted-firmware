...
/**
 * @brief Configure PLL calibration option 3
 *
 ...
 *
 * @param pll_base Base address of the PLL
 * @param enable Set to true to enable or false to disable the PLL
 */
...
/**
 * @brief Enable or disable PLL bypass mode
 *
 * @param pll Pointer to PLL context structure
 * @param enable Set to true to enable bypass, false to disable
 */
...
__wkupsramfunc int32_t k3low_pll_restore(struct pll_raw_data *pll)
{
	...
	if (pll->freq_ctrl1 == 0UL) {
		...
		while ((timeout > 0U) &&
		       (((mmio_read_32(pll->base + PLL_16FFT_CAL_STAT_OFFSET) &
