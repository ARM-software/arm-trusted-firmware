...
#include <bl32/trp/platform_trp.h>
...
static trp_args_t *trp_asc_mark_realm(unsigned long long x1)
{
	...
	if (ret != 0ULL) {
		ERROR("Granule transition from NON-SECURE type to REALM type failed 0x%llx\n", ret);
	}
	return set_smc_args(RMI_RMM_REQ_COMPLETE, 0, 0, 0, 0, 0, 0, 0);
}
...
static trp_args_t *trp_asc_mark_nonsecure(unsigned long long x1)
{
	...
	if (ret != 0ULL) {
		ERROR("Granule transition from REALM type to NON-SECURE type failed 0x%llx\n", ret);
	}
	return set_smc_args(RMI_RMM_REQ_COMPLETE, 0, 0, 0, 0, 0, 0, 0);
}
