...
#define SPM_FLAG_SRAM_SLEEP_CTRL \
	(SPM_FLAG_DISABLE_SSPM_SRAM_SLEEP | SPM_FLAG_DISABLE_SYSRAM_SLEEP)
...
bool spm_is_valid_rc_vcore(unsigned int cpu, int state_id);
int spm_update_rc_vcore(int state_id, int type, const void *val);
int spm_run_rc_vcore(unsigned int cpu, int state_id);
int spm_reset_rc_vcore(unsigned int cpu, int state_id);
int spm_get_status_rc_vcore(unsigned int type, void *priv);
