 ...
 * The PMU_CORE_PM_CON(cpu_id) reg is used to control cpu to
 * enter into suspend/reusme or off/on mode.
 * Consider the situation that cpu0 is in suspend mode,
 * but cpu1 want to power down cpu0 at the same times.
 * Although the situation do not happen on linux os.
 */
DEFINE_BAKERY_LOCK(rockchip_lock_core_pm[PLATFORM_CORE_COUNT]);

#define rockchip_core_pm_lock_get(cpu_id) \
	bakery_lock_get(&rockchip_lock_core_pm[cpu_id])
#define rockchip_core_pm_lock_release(cpu_id) \
	bakery_lock_release(&rockchip_lock_core_pm[cpu_id])
...
static int cpus_power_domain_on(uint32_t cpu_id)
{
	...
	/*
	  ...
	  */
	rockchip_core_pm_lock_get(cpu_id);
	...
	rockchip_core_pm_lock_release(cpu_id);
...
{
	uint32_t cpu_id = plat_core_pos_by_mpidr(read_mpidr_el1());
...
void plat_rockchip_pmu_init(void)
{
	...
	for (cpu = 0; cpu < PLATFORM_CORE_COUNT; cpu++)
		bakery_lock_init(&rockchip_lock_core_pm[cpu]);
