...
static struct mt_spm_cond_tables cond_vcore = {
	.table_pll = (PLL_BIT_MFG | PLL_BIT_MFGSC),
};

static struct mt_spm_cond_tables cond_vcore_res = {
	.table_pll = 0,
};
...
static struct constraint_status status = {
	...
	.enter_cnt = 0,
	.all_pll_dump = 0,
	.cond_res = &cond_vcore_res,
	.residency = 0,
};

int spm_vcore_conduct(int state_id, struct spm_lp_scen *spm_lp, uint32_t *resource_req)
{
...
bool spm_is_valid_rc_vcore(uint32_t cpu, int state_id)
{
	return (!(status.is_cond_block &&
		...
		!(vcore_ext_opand & (MT_VCORE_EXT_LP_VCORE_ON_MODE)));
}
...
int spm_update_rc_vcore(int state_id, int type, const void *val)
{
	...
	if (type == PLAT_RC_UPDATE_CONDITION) {
		...
		const struct mt_spm_cond_tables *tlb_check =
			(const struct mt_spm_cond_tables *)&cond_vcore;

		if (!tlb)
			return MT_RM_STATUS_BAD;

		status.is_cond_block = mt_spm_cond_check(
			tlb, tlb_check,
			(status.is_valid & MT_SPM_RC_VALID_COND_LATCH) ?
				&cond_vcore_res :
				NULL);
		status.all_pll_dump = mt_spm_dump_all_pll(
			tlb, tlb_check,
			(status.is_valid & MT_SPM_RC_VALID_COND_LATCH) ?
				&cond_vcore_res :
				NULL);
...
{
	uint32_t ext_op =
		(MT_SPM_EX_OP_HW_S1_DETECT | MT_SPM_EX_OP_DEVICES_SAVE |
	...
	if (IS_MT_SPM_RC_NOTIFY_ENABLE(status.is_valid))
		mt_spm_sspm_notify_u32(MT_SPM_NOTIFY_LP_ENTER, cmd);
	...
	if (IS_PLAT_SUSPEND_ID(state_id))
		...
		mt_spm_idle_generic_enter(state_id, ext_op, spm_vcore_conduct);
...
int spm_reset_rc_vcore(uint32_t cpu, int state_id)
{
	...
	uint32_t ext_op = (MT_SPM_EX_OP_HW_S1_DETECT |
			   MT_SPM_EX_OP_DEVICES_SAVE | MT_SPM_EX_OP_TRACE_LP);
	...
	if (IS_PLAT_SUSPEND_ID(state_id)) {
		...
		if (spm_unlikely(
			    !(status.is_valid & MT_SPM_RC_VALID_TRACE_EVENT)))
...
int spm_get_status_rc_vcore(uint32_t type, void *priv)
{
	...
	if (type == PLAT_RC_STATUS) {
		...
		ret = spm_rc_constraint_status_get(st->id, st->type, st->act,
						   MT_RM_CONSTRAINT_ID_VCORE,
						   &status, st->value);
