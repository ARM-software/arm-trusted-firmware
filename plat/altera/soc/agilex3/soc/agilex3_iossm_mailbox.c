...
#define ALT_FIELD_GET(_mask, _reg)					\
	({							\
		(typeof(_mask))(((_reg) & (_mask)) >> ALT_BF_SHF(_mask));	\
	})
...
void io96b_mb_init(struct io96b_info *io96b_ctrl)
{
	...
	for (i = 0; i < io96b_ctrl->num_instance; i++) {
		...
		for (k = 0; k < MAX_MEM_INTERFACES_SUPPORTED; k++) {
			switch (k) {
			case 0:
				ip_type_ret = ALT_FIELD_GET(INTF_IP_TYPE_MASK, mem_interface_0);
				instance_id_ret =
					ALT_FIELD_GET(INTF_INSTANCE_ID_MASK,
						      mem_interface_0);
				break;
			case 1:
				ip_type_ret = ALT_FIELD_GET(INTF_IP_TYPE_MASK, mem_interface_1);
				instance_id_ret =
					ALT_FIELD_GET(INTF_INSTANCE_ID_MASK,
						      mem_interface_1);
				break;
	...
	for (i = 0; i < io96b_ctrl->num_instance; i++) {
		...
		for (j = 0; j < instance->mb_ctrl.num_mem_interface; j++) {
			...
			instance->mb_ctrl.memory_size[j] =
				(ALT_FIELD_GET(INTF_CAPACITY_GBITS_MASK,
...
int ecc_enable_status(struct io96b_info *io96b_ctrl)
{
	...
	for (i = 0; i < io96b_ctrl->num_instance; i++) {
		...
		for (j = 0; j < inst_ctrl->mb_ctrl.num_mem_interface; j++) {
			...
			ecc_stat = ALT_FIELD_GET(INTF_ECC_ENABLE_TYPE_MASK,
						 ecc_enable_interface) != 0;
			is_inline_ecc = ALT_FIELD_GET(INTF_ECC_TYPE_MASK, ecc_enable_interface);
...
bool get_ecc_dbe_status(struct io96b_info *io96b_ctrl)
{
	...
	for (uint32_t i = 0; i < io96b_ctrl->num_instance; i++) {
		...
		ecc_err_counter = ALT_FIELD_GET(ECC_ERR_COUNTER_MASK, ecc_err_status);
		...
		for (uint32_t j = 0; j < ecc_err_counter && j < MAX_ECC_ERR_COUNT; j++) {
			...
			INFO("- err ip type: %lu",
				ALT_FIELD_GET(ECC_ERR_IP_TYPE_MASK, ecc_err_data));
			INFO("- err instance id: %lu",
			ALT_FIELD_GET(ECC_ERR_INSTANCE_ID_MASK, ecc_err_data));
			INFO("- err source id: %lu",
			ALT_FIELD_GET(ECC_ERR_SOURCE_ID_MASK, ecc_err_data));
			INFO("- err type: %lu", ALT_FIELD_GET(ECC_ERR_TYPE_MASK, ecc_err_data));
			INFO("- err addr upper: 0x%lx",
			ALT_FIELD_GET(ECC_ERR_ADDR_UPPER_MASK, ecc_err_data));
			INFO("- err addr lower: 0x%x", ecc_err_data_lo);

			if (is_double_bit_error(ALT_FIELD_GET(ECC_ERR_TYPE_MASK, ecc_err_data))) {
		...
		ALT_FIELD_GET(ECC_ERR_OVERFLOW_MASK, ecc_err_status));
		...
		INFO("DDR: %s: Post reset, ECC error count on IO96B_%d: %lu",
			__func__, i, ALT_FIELD_GET(ECC_ERR_COUNTER_MASK, curr_status));
	}
	...
	for (i = 0; i < io96b_ctrl->num_instance; i++) {
		...
		for (j = 0; j < inst->mb_ctrl.num_mem_interface; j++) {
			...
			while (!bist_success) {
				...
				bist_success = ALT_FIELD_GET(INTF_BIST_STATUS_MASK, status);
...
static int inline_ecc_bist_mem_init(struct io96b_info *io96b_ctrl)
{
	...
	for (i = 0; i < io96b_ctrl->num_instance; i++) {
		...
		for (j = 0; j < inst->mb_ctrl.num_mem_interface; j++) {
			...
			cmd_param_0 = ALT_FIELD_PREP(BIST_START_ADDR_SPACE_MASK, mem_exp);
			cmd_param_1 = ALT_FIELD_GET(BIST_START_ADDR_LOW_MASK, 0);
			cmd_param_2 = ALT_FIELD_GET(BIST_START_ADDR_HIGH_MASK, 0);
