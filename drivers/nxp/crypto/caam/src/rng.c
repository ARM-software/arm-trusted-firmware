...
static int is_hw_rng_instantiated(uint32_t *state_handle)
{
	...
	rdsta = sec_in32(NXP_CAAM_ADDR + RNG_REG_RDSTA_OFFSET);
...
static void kick_trng(int ent_delay)
{
	...
	val = sec_in32(NXP_CAAM_ADDR + RNG_REG_RTMCTL_OFFSET);
	val = val | RTMCTL_PRGM;
	sec_out32(NXP_CAAM_ADDR + RNG_REG_RTMCTL_OFFSET, val);

	/* rtsdctl bits 0-15 contain "Entropy Delay, which defines the
	 *  length (in system clocks) of each Entropy sample taken
	 */
	val = sec_in32(NXP_CAAM_ADDR + RNG_REG_RTSDCTL_OFFSET);
	val = (val & ~RTSDCTL_ENT_DLY_MASK) |
	    (ent_delay << RTSDCTL_ENT_DLY_SHIFT);
	sec_out32(NXP_CAAM_ADDR + RNG_REG_RTSDCTL_OFFSET, val);
	/* min. freq. count, equal to 1/4 of the entropy sample length */
	sec_out32(NXP_CAAM_ADDR + RNG_REG_RTFRQMIN_OFFSET, ent_delay >> 2);
	/* disable maximum frequency count */
	sec_out32(NXP_CAAM_ADDR + RNG_REG_RTFRQMAX_OFFSET, RTFRQMAX_DISABLE);
