...
static int spm_ap_mdsrc_ack(void)
{
	...
	if (!ack) {
		...
		md_state = !!(mmio_read_32(SPM_REQ_STA_9) & MD_APSRC_REQ_LSB);
...
struct mt_lp_res_req *spm_resources[] = {
	&rq_xo_fpm,  &rq_26m,	&rq_infra, &rq_syspll, &rq_dram_s0,
	&rq_dram_s1, &rq_vcore, &rq_emi,   &rq_pmic,   NULL,
};

struct mt_resource_req_manager plat_mt8189_rq = {
	.res = spm_resources,
};
...
int mt_spm_hwctrl(uint32_t type, int set, void *priv)
{
	...
	switch (type) {
	case PLAT_AP_MDSRC_REQ:
		spm_ap_mdsrc_req(set);
		break;
	case PLAT_AP_MDSRC_ACK:
		ret = spm_ap_mdsrc_ack();
		break;
	case PLAT_AP_IS_MD_SLEEP:
		ret = spm_is_md_sleep(priv);
		break;
	case PLAT_AP_MDSRC_SETTLE:
		if (!priv)
			return -1;
		*(int *)priv = AP_MDSRC_REQ_MD_26M_SETTLE;
		break;
	case PLAT_AP_GPUEB_PLL_CONTROL:
		spm_ap_gpueb_pll_control(set);
		break;
	case PLAT_AP_GPUEB_PWR_STATUS:
		if (!priv)
			return -1;
		*(uint32_t *)priv = spm_ap_gpueb_get_pwr_status();
		break;
	case PLAT_AP_GPUEB_MFG0_PWR_CON:
		if (!priv)
			return -1;
...
#ifndef MTK_PLAT_SPM_UNSUPPORT
struct mt_resource_manager plat_mt8189_rm = {
	.update = NULL,
...
int spm_boot_init(void)
{
	...
	mt_lp_rm_register(&plat_mt8189_rm);

/* If spm service won't run when spm not ready */
#ifndef MTK_PLAT_SPM_UNSUPPORT
	mt_lp_resource_request_manager_register(&plat_mt8189_rq);
