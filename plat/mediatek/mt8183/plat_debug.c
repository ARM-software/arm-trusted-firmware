...
void circular_buffer_setup(void)
{
	...
	sync_writel(CA15M_DBG_CONTROL, mmio_read_32(CA15M_DBG_CONTROL) & ~(BIT_CA15M_LASTPC_DIS));
}
...
void circular_buffer_unlock(void)
{
	...
	for (i = 1; i <= 4; ++i)
		sync_writel(MP1_CPUTOP_PWR_CON + i * 4, (mmio_read_32(MP1_CPUTOP_PWR_CON + i * 4) & ~(0x4))|(0x4));
...
void l2c_parity_check_setup(void)
{
	...
	sync_writel(CA15M_DBG_CONTROL, mmio_read_32(CA15M_DBG_CONTROL) | BIT_CA15M_L2PARITY_EN);
}
