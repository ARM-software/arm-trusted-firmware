...
int add_pbi_stop_cmd(FILE *fp_rcw_pbi_op)
{
	...
	case CHASSIS_3:
	case CHASSIS_3_2:
		/* TBD - add a flag. based on flag add the corresponding cmd
...
int main(int argc, char **argv)
{
	...
	while ((opt = getopt(argc, argv,
		...
		case 'd':
			...
			if (*ptr) {
				fprintf(stderr, "CMD Error: invalid \
					load/destination address %s\n", optarg);
				goto exit_main;
			}
		...
		case 'b':
			...
			else if (!strcmp(optarg, "flexspi_nor"))
				pblimg.boot_src = FLXSPI_NOR_BOOT;
			else if (!strcmp(optarg, "flexspi_nand"))
	...
	case CHASSIS_3_2:
		...
		while (word != 0x808f0000 && word != 0x80ff0000) {
			...
			/* TBD -- Check load command..
			 * add a check if command is Stop with CRC or
			 * stop without checksum
			 */
			if (pbl_size == 35) {
