...
# Path to the global configuration header, generated by the build system
config-header = $(abspath $(BUILD_PLAT))/config.h
...
# Let the shell interpret the existing `-D` quoting once, then convert each
# argument to a `#define`. Preserve the header's timestamp when unchanged.
$(config-header): FORCE | $$(@D)/
	$(q)printf '%s\n' $(DEFINES) | sed \
		-e '/^-D$$/{N;s/\n//;}' \
		-e 's/^-D\([^=]*\)$$/#define \1 1/' \
		-e 's/^-D\([^=]*\)=/#define \1 /' > $(call shell-quote,$@.tmp)

	$(q)if cmp -s $(call shell-quote,$@.tmp) $(call shell-quote,$@); then \
		rm -f $(call shell-quote,$@.tmp); \
	else \
		mv -f $(call shell-quote,$@.tmp) $(call shell-quote,$@); \
	fi
