 ...
 *
 * This header provides device preparation functions for TISCI requests,
 * including validation of device access permissions, exclusive ownership
 * checks, and host index/device lookup resolution.
 */
 ...
 *
 * \return
 * 0 if all checks succeeded, less than zero otherwise.
 */
 ...
 * 0 if all checks succeeded, less than zero otherwise.
 */
int32_t device_prepare_nonexclusive(uint8_t host_id, uint32_t id,
				    uint8_t *host_idx,
				    struct ti_device **device_ptr);
